...
face_cascade = cv2.CascadeClassifier(r'C:\Users\sgupt\Documents\faceunlock\Lock-Unlock-Laptop-PC-Screen-Using-Face-Recognition\haarcascade_frontalface_default.xml') # Detect object in video stream using Haarcascade Frontal Face

face_id = 1  # For each person,there will be one face id
count = 0    # Initialize sample face image

check_path(r"C:\Users\sgupt\Documents\faceunlock\dataset")
...
while(True):
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite(r"C:\Users\sgupt\Documents\faceunlock\dataset" + str(face_id) + '.' + str(count) + ".jpg", gray[y:y+h,x:x+w]) # Save the captured image into the datasets folder
